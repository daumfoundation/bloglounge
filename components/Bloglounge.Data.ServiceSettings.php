<?php
	class ServiceSettings {
		...
		function ServiceSettings() {
			global $database, $db;
			if ($data = $db->queryAll('SELECT * FROM '.$database['prefix'].'ServiceSettings',MYSQL_ASSOC)) {
		...
		function get($name) {
			global $database, $db;
			list($result) = $db->pick('SELECT value FROM '.$database['prefix'].'ServiceSettings WHERE name = "'.$name.'" LIMIT 1');
			return $result;
		}
		...
		function set($name, $value) {
			global $database, $db;
			return $db->execute('UPDATE '.$database['prefix'].'ServiceSettings SET value="'.$db->escape($value).'" WHERE name = "'.$name.'"');
		}
